...
void PinweaverModel::ApplyRequest(const fuzz::pinweaver::Request& pinweaver,
                                  fuzz::span<uint8_t> buffer) {
  ...
  if (pinweaver.request_case() ==  fuzz::pinweaver::Request::kInsertLeaf) {
    ...
    std::copy(insert.reset_secret,
              insert.reset_secret + PW_SECRET_SIZE,
  ...
  switch(pinweaver.request_case()) {
    case  fuzz::pinweaver::Request::kResetTree:
      ...
      break;
    case  fuzz::pinweaver::Request::kInsertLeaf:
      ApplyInsertLeaf(pinweaver, buffer, &leaf_data);
      break;
    case  fuzz::pinweaver::Request::kRemoveLeaf:
...
size_t PinweaverModel::CopyMetadata(
    uint64_t label, const LeafData& leaf_data,
...
size_t PinweaverModel::GetMetadata(
  ...
  if (itr == leaf_metadata_.end()) {
    // There is no metadata present so return an empty payload. This is ok
    // because there are unimported_leaf_data fields in the TryAuth and
    // ResetAuth protos that take the place of this field if they are set.
...
size_t PinweaverModel::SerializeInsertLeaf(
  ...
  fuzz::CopyWithPadding(
      ...
      fuzz::span<uint8_t>(reinterpret_cast<uint8_t*>(req_data->delay_schedule),
                          sizeof(req_data->delay_schedule)), 0);
  fuzz::CopyWithPadding(
...
void PinweaverModel::UpdateMetadata(
    ...
    size_t unimported_leaf_data_length, const LeafData* leaf_data) {
  ...
  if (unimported_leaf_data) {
    ...
    stored_leaf_data.wrapped_data.assign(
        data, data + unimported_leaf_data_length);
...
void PinweaverModel::ApplyResetTree(const fuzz::pinweaver::Request& pinweaver,
                                    fuzz::span<uint8_t> buffer) {
  leaf_metadata_.clear();
...
void PinweaverModel::ApplyRemoveLeaf(const fuzz::pinweaver::Request& pinweaver,
  ...
  pw_response_t* response = reinterpret_cast<pw_response_t*>(buffer.begin());
  UpdateMetadata(
      pinweaver.remove_leaf().label(), &response->header,
      nullptr /*unimported_leaf_data*/, 0 /*unimported_leaf_data_length*/,
      nullptr /*leaf_data*/);
}
