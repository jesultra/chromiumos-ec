 ...
 * image. Output in a machine-friendly format if show_machine_output != 0.
 */
static int show_headers_versions(const void *image,
				 const int show_machine_output)
{
	...
	const size_t num_section_per_slot = 2;

	// String representation of FW version (<epoch>:<major>:<minor>), one
	// string for each FW section.
	char ro_fw_ver[num_slots][MAX_FW_VER_LENGTH];
	char rw_fw_ver[num_slots][MAX_FW_VER_LENGTH];
	...
	for (i = 0; i < ARRAY_SIZE(sections); i++) {
		...
		const size_t slot_idx = i / num_section_per_slot;
