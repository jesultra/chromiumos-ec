...
/* TODO: Delete this. */
#define I2C_PSEUDO_ENABLE_RSP_ERROR_FORMATTER
#undef I2C_PSEUDO_ENABLE_RSP_ERROR_FORMATTER
...
#define I2C_PSEUDO_TIMEOUT_MS_MIN 0
#define I2C_PSEUDO_TIMEOUT_MS_MAX (60 * 1000)
#define I2C_PSEUDO_DEFAULT_TIMEOUT_MS (3 * 1000)
...
#define I2C_PSEUDO_ADAP_START_CMD "ADAPTER_START"
#define I2C_PSEUDO_ADAP_SHUTDOWN_CMD "ADAPTER_SHUTDOWN"
#define I2C_PSEUDO_GET_NUMBER_CMD "GET_ADAPTER_NUM"
#define I2C_PSEUDO_NUMBER_REPLY_CMD "I2C_ADAPTER_NUM"
#define I2C_PSEUDO_SET_NAME_SUFFIX_CMD "SET_ADAPTER_NAME_SUFFIX"
#define I2C_PSEUDO_SET_TIMEOUT_CMD "SET_ADAPTER_TIMEOUT_MS"
...
#define I2C_PSEUDO_CTRLR_ERROR_CMD "CONTROLLER_ERROR"
#endif
 ...
 */
static unsigned int i2c_pseudo_default_timeout_ms =
	I2C_PSEUDO_DEFAULT_TIMEOUT_MS;
module_param(i2c_pseudo_default_timeout_ms, uint, S_IRUGO);

/*
 * This should be an unsigned type large enough to hold I2C_PSEUDO_ADAPTERS_MAX.
 * If changing this type, audit for printf format strings that need updating!
 */
...
/*
 * An instance of this struct in i2c_pseudo_cmds[] array defines a command that
 * a controller process may write to the I2C pseudo character device, hereafter
 * a "write command."
 *
 ...
 *
 * A header field ends at either i2c_pseudo_ctrlr_end_char or
 * i2c_pseudo_ctrlr_header_sep_char, neither of which is ever included in header
 * field values passed to a callback.
 *
 * A command always ends at i2c_pseudo_ctrlr_end_char.  Anything written after
 * that by the controller is treated as a new command.
 *
 * After i2c_pseudo_ctrlr_header_sep_char the return value of header_receiver()
 * from the previous header field is used to determine whether subsequent input
 * is another header field, or data.
 *
 * Once header_receiver() has indicated that data is expected, all input until
 * i2c_pseudo_ctrlr_end_char will be handled as data, and header_receiver() will
 * not be called again for the command.
 *
 ...
 * cross-thread synchronization themselves.
 *
 * Note: Data may contain i2c_pseudo_ctrlr_header_sep_char.
 *
 * Note: There are no restrictions on the use of the null char ('\0') in either
 * header fields or data.  (If either i2c_pseudo_ctrlr_header_sep_char or
 * i2c_pseudo_ctrlr_end_char is null then the respective restrictions around
 * those characters apply as usual, of course.)  Write command implementations
 * need not use or expect null, but they still must handle it gracefully and
 * fail without bad side effects, same as with any unexpected input.
 */
struct i2c_pseudo_cmd {
	/*
	 * Set these to the command name.
	 *
	 * The command name must not contain i2c_pseudo_ctrlr_header_sep_char or
	 * i2c_pseudo_ctrlr_end_char.  The behavior otherwise is undefined; such
	 * a command would be uncallable, and could become either a build-time
	 * or runtime error.
	 *
	 * The command name must be unique in the i2c_pseudo_cmds[] array.  The
	 ...
	 * i2c_pseudo_ctrlr_header_sep_char or i2c_pseudo_ctrlr_end_char.
	 *
	 ...
	 * i2c_pseudo_ctrlr_end_char could be received on a non-chunk-size
	 * boundary.  The return value should be less than
	 * I2C_PSEUDO_CTRLR_CMD_BUF_SIZE, as that minus one is the maximum that
	 * will ever be buffered at once, and thus the maximum that will ever be
	 * sent to a single invocation of data_receiver.
	 *
	 ...
	 * either the chunk size is >= I2C_PSEUDO_CTRLR_CMD_BUF_SIZE, or
	 * i2c_pseudo_ctrlr_end_char was reached on a non-chunk-sized boundary.
	 *
	 ...
	 * field >= I2C_PSEUDO_CTRLR_CMD_BUF_SIZE.
	 *
	 ...
	 * This callback may be NULL.
	 */
	int (*cmd_completer)(void *data, struct i2c_pseudo_controller *pdata,
...
#endif
	I2C_PSEUDO_CMD_ADAP_START_IDX,
	I2C_PSEUDO_CMD_ADAP_SHUTDOWN_IDX,
	I2C_PSEUDO_CMD_GET_NUMBER_IDX,
	I2C_PSEUDO_CMD_SET_NAME_SUFFIX_IDX,
	I2C_PSEUDO_CMD_SET_TIMEOUT_IDX,
	/* Keep this at the end!  This must equal ARRAYLEN(i2c_pseudo_cmds). */
...
enum i2c_pseudo_ctrlr_state {
	 ...
	 * controller.
	 */
	I2C_PSEUDO_CTRLR_STATE_SHUTDN_REQUESTED,
};
...
struct i2c_pseudo_controller {
	...
	/*
	 * Only i2c_pseudo_cdev_open() and i2c_pseudo_cdev_release() may access
	 * this field.  Other functions called by them, or called by the I2C
	 * subsystem, may of course take a reference to this same struct
	 * i2c_adapter.  However no other functions besides the aforementioned
	 * two may access the i2c_adapter field of struct i2c_pseudo_controller.
	 */
	struct i2c_adapter i2c_adapter;

	struct mutex startstop_lock;
	enum i2c_pseudo_ctrlr_state startstop_state;
	...
	void *cmd_data[I2C_PSEUDO_NUM_WRITE_CMDS];
...
struct i2c_pseudo_cmd_set_name_suffix_data {
	int field_pos;
	char name_suffix[sizeof(((struct i2c_adapter *)0)->name)];
	size_t name_suffix_len;
};

struct i2c_pseudo_cmd_set_timeout_data {
	int field_pos;
...
struct i2c_pseudo_rsp {
	 ...
	 *
	 * A negative return value indicates an error occured and the data
	 ...
	 *
	 * The formatter must NOT use i2c_pseudo_ctrlr_end_char in anywhere in
	 * *out (within the size range indicated by the return value; past that
	 * does not matter).  The i2c_pseudo_ctrlr_end_char will be added
	 * automatically by the caller after a zero return value (successful
	 * completion) from the formatter.
	 *
	 * The formatter must never create or return a buffer larger than
	 * I2C_PSEUDO_MAX_MSG_BUF_SIZE.  The formatter is encouraged to avoid
	 * that by generating and returning the output in chunks, taking
	 * advantage of the guarantee that it will be called repeatedly until
	 * exhaustion (zero return value) or failure (negative return value).
	 * If the formatter expects its formatted output or natural subsets of
	 * it to always fit within I2C_PSEUDO_MAX_MSG_BUF_SIZE, and it is called
	 * with input data not meeting that expectation, the formatter should
	 * return -ERANGE to indicate this condition.
	 */
...
#ifdef I2C_PSEUDO_ENABLE_RSP_ERROR_FORMATTER
...
static ssize_t i2c_pseudo_rsp_buffer_formatter(void *data, char **out)
{
	...
	if (rsp_buf->buf != NULL) {
		if (rsp_buf->size > 0) {
			*out = rsp_buf->buf;
			rsp_buf->buf = NULL;
			return min(rsp_buf->size, (size_t)SSIZE_T_MAX);
		}
		kfree(rsp_buf->buf);
	}
	kfree(rsp_buf);
	...
	if (ret > 0) {
		*out = buf;
		...
		pr_debug("%s: Unexpectedly received zero formattered "
			"characters / one buffer size (including null "
...
static ssize_t i2c_pseudo_rsp_master_xfer_formatter(void *data, char **out)
{
 ...
 early_error:
		if (mxfer_rsp->num_msgs_done >= mxfer_rsp->num) {
 /* Jump destination for when we already know the conditions above are true. */
...
static ssize_t i2c_pseudo_id_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	...
	pdata = container_of(adap, struct i2c_pseudo_controller, i2c_adapter);
	ret = snprintf(buf, PAGE_SIZE, "%u\n", pdata->id);
	if (ret >= PAGE_SIZE)
		return -ERANGE;
	return i2c_pseudo_int_to_ssize_t(ret);
}

static const struct device_attribute i2c_pseudo_id_dev_attr = {
	.attr = {
		.name = "i2c-pseudo-id",
		.mode = S_IRUGO,
	},
	.show = i2c_pseudo_id_show,
};

static int i2c_pseudo_adap_get_state(struct i2c_pseudo_controller *pdata)
{
	int ret;
	if (mutex_lock_killable(&pdata->startstop_lock)) {
		ret = -EINTR;
		pr_warn("%s: I2C pseudo ID %u startstop_lock acquisition "
...
static int i2c_pseudo_cmd_mxfer_reply_data_creator(void **data)
{
...
static void i2c_pseudo_cmd_mxfer_reply_data_destroyer(void *data)
{
	/*
...
static int i2c_pseudo_cmd_mxfer_reply_cmd_completer(void *data,
	struct i2c_pseudo_controller *pdata, int receive_status,
	bool non_blocking)
{
	int ret;
	struct i2c_pseudo_cmd_mxfer_reply_data *cmd_data;
...
static int i2c_pseudo_cmd_adap_start_header_receiver(void *data, char *in,
	size_t in_size, bool non_blocking)
{
	if (STRLEN(I2C_PSEUDO_ADAP_START_CMD) != in_size ||
	    memcmp(I2C_PSEUDO_ADAP_START_CMD, in, in_size))
		/* Reaching here is a bug. */
		return -ENOTRECOVERABLE;
	/*
...
static int i2c_pseudo_cmd_adap_start_data_receiver(void *data, char *in,
	size_t in_size, bool non_blocking)
{
	/*
...
static int i2c_pseudo_cmd_adap_start_cmd_completer(void *data,
	struct i2c_pseudo_controller *pdata, int receive_status,
	bool non_blocking)
{
	int ret;
	 ...
	 * i2c_pseudo_adap_get_state() in order to keep the lock while calling
	 * i2c_add_adapter().
	 */
	if (mutex_lock_killable(&pdata->startstop_lock)) {
		ret = -EINTR;
	...
	if (pdata->startstop_state != I2C_PSEUDO_CTRLR_STATE_NEW) {
		ret = -EISCONN;
		pr_err("%s: I2C pseudo ID %u controller attempted to start "
			"already-started I2C adapter, returning %d.\n",
			__func__, pdata->id, ret);
		goto unlock_and_return;
	}
	...
	if (ret < 0) {
		pr_err("%s: i2c_add_adapter() for I2C pseudo ID %u failed with "
			"error %d\n", __func__, pdata->id, -ret);
		goto unlock_and_return;
	}

	pdata->startstop_state = I2C_PSEUDO_CTRLR_STATE_RUNNING;

	/* Add the I2C pseudo controller ID sysfs file. */
	ret = device_create_file(&pdata->i2c_adapter.dev,
		&i2c_pseudo_id_dev_attr);
	if (ret < 0) {
		pr_err("%s: device_create_file(&pdata->i2c_adapter.dev, "
			"&i2c_pseudo_id_dev_attr) failed with error %d\n",
			__func__, -ret);
		goto unlock_and_return;
	}
...
static int i2c_pseudo_cmd_adap_shutdown_header_receiver(void *data, char *in,
	size_t in_size, bool non_blocking)
{
	if (STRLEN(I2C_PSEUDO_ADAP_SHUTDOWN_CMD) != in_size ||
	    memcmp(I2C_PSEUDO_ADAP_SHUTDOWN_CMD, in, in_size))
		/* Reaching here is a bug. */
		return -ENOTRECOVERABLE;
	/*
	 * No more header fields or data are expected.  This directs any further
	...
	bool non_blocking)
{
	...
	pdata->startstop_state = I2C_PSEUDO_CTRLR_STATE_SHUTDN_REQUESTED;
	mutex_unlock(&pdata->startstop_lock);
...
static int i2c_pseudo_cmd_get_number_header_receiver(void *data, char *in,
	size_t in_size, bool non_blocking)
{
	if (STRLEN(I2C_PSEUDO_GET_NUMBER_CMD) != in_size ||
	    memcmp(I2C_PSEUDO_GET_NUMBER_CMD, in, in_size)) {
		/* Reaching here is a bug. */
		return -ENOTRECOVERABLE;
	}
	/*
...
static int i2c_pseudo_cmd_get_number_data_receiver(void *data, char *in,
	size_t in_size, bool non_blocking)
{
	/*
	 * Reaching here means the controller wrote extra data in the command
...
static int i2c_pseudo_cmd_get_number_cmd_completer(void *data,
	struct i2c_pseudo_controller *pdata, int receive_status,
	bool non_blocking)
{
	ssize_t ret = 0;
	struct i2c_pseudo_rsp_buffer *rsp_buf;
	struct i2c_pseudo_rsp *rsp_wrapper;
	...
	ret = anprintf(&rsp_buf->buf, I2C_PSEUDO_MAX_MSG_BUF_SIZE, "%*s%c%d",
		(int)STRLEN(I2C_PSEUDO_NUMBER_REPLY_CMD),
		I2C_PSEUDO_NUMBER_REPLY_CMD, i2c_pseudo_ctrlr_header_sep_char,
	...
	if (mutex_lock_killable(&pdata->read_rsp_queue_lock)) {
		ret = -EINTR;
		pr_warn("%s: I2C pseudo ID %u read_rsp_queue_lock acquisition "
	...
	if (pdata->read_rsp_queue_length >= I2C_PSEUDO_CTRLR_RSP_QUEUE_LIMIT) {
		ret = -ENOBUFS;
		pr_debug("%s: I2C pseudo ID %u has reached "
			STR(I2C_PSEUDO_CTRLR_RSP_QUEUE_LIMIT) " request queue "
			"limit.  Returning %zd now.\n", __func__, pdata->id,
			ret);
 ...
 fail_after_rsp_wrapper_alloc:
	kfree(rsp_wrapper);
 just_return:
	return i2c_pseudo_ssize_t_to_int(ret);
}

static int i2c_pseudo_cmd_set_name_suffix_data_creator(void **data)
{
	struct i2c_pseudo_cmd_set_name_suffix_data *cmd_data;
	cmd_data = kzalloc(sizeof(*cmd_data), GFP_KERNEL);
	if (cmd_data == NULL) {
		pr_err("%s: kzalloc(%zu, GFP_KERNEL) returned NULL\n", __func__,
			sizeof(*cmd_data));
...
static void i2c_pseudo_cmd_set_name_suffix_data_destroyer(void *data)
{
	kfree(data);
}

static int i2c_pseudo_cmd_set_name_suffix_header_receiver(void *data, char *in,
	size_t in_size, bool non_blocking)
{
	struct i2c_pseudo_cmd_set_name_suffix_data *cmd_data;
	cmd_data = data;
	...
	case 0:
		if (STRLEN(I2C_PSEUDO_SET_NAME_SUFFIX_CMD) != in_size ||
		    memcmp(I2C_PSEUDO_SET_NAME_SUFFIX_CMD, in, in_size))
			/* Reaching here is a bug. */
			return -ENOTRECOVERABLE;
		return 0;
	case 1:
		...
		if (in_size > sizeof(cmd_data->name_suffix))
			in_size = sizeof(cmd_data->name_suffix);
		memcpy(cmd_data->name_suffix, in, in_size);
...
static int i2c_pseudo_cmd_set_name_suffix_data_receiver(void *data, char *in,
	size_t in_size, bool non_blocking)
{
	/*
	 * Reaching here means the controller wrote extra data in the command
...
static int i2c_pseudo_cmd_set_name_suffix_cmd_completer(void *data,
	struct i2c_pseudo_controller *pdata, int receive_status,
	bool non_blocking)
{
	int ret;
	...
	if (ret < 0) {
		pr_err("%s: snprintf() for I2C pseudo ID %u i2c_adapter.name "
			"failed with error %d\n", __func__, pdata->id, -ret);
		return ret;
	}
	return 0;
...
static int i2c_pseudo_cmd_set_timeout_data_creator(void **data)
{
	struct i2c_pseudo_cmd_set_timeout_data *cmd_data;
	cmd_data = kzalloc(sizeof(*cmd_data), GFP_KERNEL);
	if (cmd_data == NULL) {
		pr_err("%s: kzalloc(%zu, GFP_KERNEL) returned NULL\n", __func__,
			sizeof(*cmd_data));
...
static void i2c_pseudo_cmd_set_timeout_data_destroyer(void *data)
{
	kfree(data);
}

static int i2c_pseudo_cmd_set_timeout_header_receiver(void *data, char *in,
	size_t in_size, bool non_blocking)
{
	int ret;
	struct i2c_pseudo_cmd_set_timeout_data *cmd_data;
	...
	switch (cmd_data->field_pos++) {
	case 0:
		if (STRLEN(I2C_PSEUDO_SET_TIMEOUT_CMD) != in_size ||
		    memcmp(I2C_PSEUDO_SET_TIMEOUT_CMD, in, in_size))
			/* Reaching here is a bug. */
			return -ENOTRECOVERABLE;
		return 0;
	case 1:
...
static int i2c_pseudo_cmd_set_timeout_data_receiver(void *data, char *in,
	size_t in_size, bool non_blocking)
{
	/*
	 * Reaching here means the controller wrote extra data in the command
...
static int i2c_pseudo_cmd_set_timeout_cmd_completer(void *data,
	struct i2c_pseudo_controller *pdata, int receive_status,
	bool non_blocking)
{
	struct i2c_pseudo_cmd_set_timeout_data *cmd_data;
	...
	if (receive_status)
		return 0;
	cmd_data = data;
	if (cmd_data->timeout_ms < I2C_PSEUDO_TIMEOUT_MS_MIN ||
		cmd_data->timeout_ms > I2C_PSEUDO_TIMEOUT_MS_MAX)
		return -ERANGE;
	pdata->i2c_adapter.timeout = msecs_to_jiffies(cmd_data->timeout_ms);
	return 0;
}

/* Commands are looked up in this order, so sort this by expected frequency. */
static const struct i2c_pseudo_cmd i2c_pseudo_cmds[] = {
	{
		...
		.data_creator = i2c_pseudo_cmd_mxfer_reply_data_creator,
		.data_destroyer = i2c_pseudo_cmd_mxfer_reply_data_destroyer,
...
/* TODO: Implement handling of an error write command from the controller. */
...
#endif
	{
		.cmd_string = I2C_PSEUDO_ADAP_START_CMD,
		.cmd_size = STRLEN(I2C_PSEUDO_ADAP_START_CMD),
		.header_receiver = i2c_pseudo_cmd_adap_start_header_receiver,
		.data_receiver = i2c_pseudo_cmd_adap_start_data_receiver,
		.cmd_completer = i2c_pseudo_cmd_adap_start_cmd_completer,
	},
	{
	...
	{
		.cmd_string = I2C_PSEUDO_GET_NUMBER_CMD,
		.cmd_size = STRLEN(I2C_PSEUDO_GET_NUMBER_CMD),
		.header_receiver = i2c_pseudo_cmd_get_number_header_receiver,
		.data_receiver = i2c_pseudo_cmd_get_number_data_receiver,
		.cmd_completer = i2c_pseudo_cmd_get_number_cmd_completer,
	},
	{
		.cmd_string = I2C_PSEUDO_SET_NAME_SUFFIX_CMD,
		.cmd_size = STRLEN(I2C_PSEUDO_SET_NAME_SUFFIX_CMD),
		.data_creator = i2c_pseudo_cmd_set_name_suffix_data_creator,
		.data_destroyer = i2c_pseudo_cmd_set_name_suffix_data_destroyer,
		.header_receiver =
			i2c_pseudo_cmd_set_name_suffix_header_receiver,
		.data_receiver = i2c_pseudo_cmd_set_name_suffix_data_receiver,
		.cmd_completer = i2c_pseudo_cmd_set_name_suffix_cmd_completer,
	},
	{
		.cmd_string = I2C_PSEUDO_SET_TIMEOUT_CMD,
		.cmd_size = STRLEN(I2C_PSEUDO_SET_TIMEOUT_CMD),
		.data_creator = i2c_pseudo_cmd_set_timeout_data_creator,
		.data_destroyer = i2c_pseudo_cmd_set_timeout_data_destroyer,
		.header_receiver = i2c_pseudo_cmd_set_timeout_header_receiver,
		.data_receiver = i2c_pseudo_cmd_set_timeout_data_receiver,
		.cmd_completer = i2c_pseudo_cmd_set_timeout_cmd_completer,
	},
};
	...
	struct i2c_msg *msgs, int num)
{
	...
	i = i2c_pseudo_adap_get_state(pdata);
	if (i < 0) {
		ret = i;
		goto just_return;
	}
	if (i == I2C_PSEUDO_CTRLR_STATE_SHUTDN_REQUESTED)
		goto just_return;
...
inline static void i2c_pseudo_check_for_cdev_release_race(
	struct i2c_pseudo_controller *pdata)
{
	if (mutex_trylock(&pdata->read_rsp_queue_lock))
		mutex_unlock(&pdata->read_rsp_queue_lock);
	else
		pr_err("%s: Could not immediately acquire I2C pseudo ID %u "
			"read_rsp_queue_lock.\n", __func__, pdata->id);
...
static int i2c_pseudo_cdev_release(struct inode *inodep, struct file *filep)
{
	 ...
	 * completely undefined state referencing free or re-allocated memory.
	 */
	wake_up_interruptible_all(&pdata->poll_wait_queue);
#ifdef I2C_PSEUDO_CHECK_FOR_CDEV_RELEASE_RACE
	i2c_pseudo_check_for_cdev_release_race(pdata);
#endif
	/*
	...
	/*
	 * It should not be possible for pdata->startstop_state to change at
	 * this point, however the memory barrier from acquiring the lock could
	 * be useful.
	 */
		...
		size_t count, loff_t *f_ps)
{
	...
	if (non_blocking) {
		if (!mutex_trylock(&pdata->rsp_lock)) {
			BUILD_BUG_ON(ret != 0);
			ret = -EAGAIN;
			pr_debug("%s: Could not immediately acquire I2C pseudo "
				"ID %u rsp_lock for O_NONBLOCK read, returning "
				"%zd.\n", __func__, pdata->id, ret);
			goto just_return;
		}
	} else {
		if (mutex_lock_killable(&pdata->rsp_lock)) {
			BUILD_BUG_ON(ret != 0);
			ret = -EINTR;
			pr_warn("%s: I2C pseudo ID %u rsp_lock acquisition "
				"interrupted by signal, returning %zd.\n",
				__func__, pdata->id, ret);
			goto just_return;
		}
	...
	while (count > 0) {
		...
		while (pdata->rsp_buf_remaining == 0) {
			...
			mutex_unlock(&pdata->read_rsp_queue_lock);
			if (rsp_wrapper == NULL) {
				int_ret = i2c_pseudo_adap_get_state(pdata);
				if (int_ret < 0) {
					if (ret == 0)
	...
	if (copy_size > buf_remaining) {
		...
		if (!pdata->cmd_receive_status) {
			pr_warn("%s: Error: Exceeded maximum %zu size of I2C "
				...
				"pseudo ID %u.  The command currently being "
				"written will be ignored.\n", __func__,
				sizeof(pdata->cmd_buf), pdata->id);
		...
		size_t count, loff_t *f_ps)
{
	...
	if (non_blocking) {
		if (!mutex_trylock(&pdata->cmd_lock)) {
			...
			ret = -EAGAIN;
			pr_debug("%s: Could not immediately acquire I2C pseudo "
				"ID %u cmd_lock for O_NONBLOCK write, "
				"returning %zd.\n", __func__, pdata->id, ret);
			goto free_and_return;
		}
	} else {
		if (mutex_lock_killable(&pdata->cmd_lock)) {
			BUILD_BUG_ON(ret != 0);
			ret = -EINTR;
			pr_warn("%s: I2C pseudo ID %u cmd_lock acquisition "
				"interrupted by signal, returning %zd.\n",
				__func__, pdata->id, ret);
			goto free_and_return;
		}
...
static __poll_t i2c_pseudo_cdev_poll(struct file *filep, poll_table *ptp)
{
	...
	if (int_ret < 0)
		 ...
		 * from i2c_pseudo_adap_get_state() indicates.  POLLERR is
...
static ssize_t i2c_pseudo_limit_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int ret;
	ret = snprintf(buf, PAGE_SIZE, "%u\n", i2c_pseudo_limit);
...
static const struct device_attribute i2c_pseudo_limit_dev_attr = {
	.attr = {
		.name = "limit",
		.mode = S_IRUGO,
	},
	.show = i2c_pseudo_limit_show,
};

static ssize_t i2c_pseudo_count_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int count, ret;
	if (mutex_lock_killable(&i2c_pseudo_counters.lock)) {
		pr_warn("%s: Request for new pseudo I2C adapter lock "
			"acquisition interrupted by signal, returning -EINTR."
			"\n", __func__);
		return -EINTR;
	}
	count = i2c_pseudo_counters.count;
	mutex_unlock(&i2c_pseudo_counters.lock);
	ret = snprintf(buf, PAGE_SIZE, "%u\n", count);
	if (ret >= PAGE_SIZE)
		return -ERANGE;
...
static const struct device_attribute i2c_pseudo_count_dev_attr = {
	.attr = {
		.name = "count",
		.mode = S_IRUGO,
	},
	.show = i2c_pseudo_count_show,
};
...
inline static void i2c_p_device_remove_file_count(void)
{
	device_remove_file(&i2c_pseudo_device, &i2c_pseudo_count_dev_attr);
}

inline static void i2c_p_device_remove_file_limit(void)
{
	device_remove_file(&i2c_pseudo_device, &i2c_pseudo_limit_dev_attr);
}
...
static int __init i2c_pseudo_init(void)
{
	...
	/* Add the I2C pseudo controller limit sysfs file. */
	ret = device_create_file(&i2c_pseudo_device,
		&i2c_pseudo_limit_dev_attr);
	if (ret < 0) {
		pr_err("%s: device_create_file(&i2c_pseudo_device, "
			"&i2c_pseudo_limit_dev_attr) failed with error %d\n",
			__func__, -ret);
		goto fail_after_device_add;
	}

	/* Add the I2C pseudo controller count sysfs file. */
	ret = device_create_file(&i2c_pseudo_device,
		&i2c_pseudo_count_dev_attr);
	if (ret < 0) {
		pr_err("%s: device_create_file(&i2c_pseudo_device, "
			"&i2c_pseudo_count_dev_attr) failed with error %d\n",
			__func__, -ret);
		goto fail_after_limit_sysfs_create;
	}
 ...
 fail_after_limit_sysfs_create:
	i2c_p_device_remove_file_limit();
 fail_after_device_add:
	i2c_p_device_delete();
...
inline static void i2c_pseudo_build_assertions(void)
{
	...
	BUILD_BUG_ON(I2C_PSEUDO_DEFAULT_TIMEOUT_MS < I2C_PSEUDO_TIMEOUT_MS_MIN);
	BUILD_BUG_ON(I2C_PSEUDO_DEFAULT_TIMEOUT_MS > I2C_PSEUDO_TIMEOUT_MS_MAX);
	...
	/*
	 * Some parts of this module assume this field is a fixed length array.
	 * Attempt to fail if this field is ever changed to char * .
	 */
	BUILD_BUG_ON(sizeof(((struct i2c_adapter *)0)->name) <= sizeof(char *));
}

static void __exit i2c_pseudo_exit(void)
{
	i2c_p_device_remove_file_count();
	i2c_p_device_remove_file_limit();
	i2c_p_device_delete();
	i2c_p_device_put();
	i2c_p_cdev_delete();
