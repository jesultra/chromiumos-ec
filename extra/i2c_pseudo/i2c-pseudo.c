 ...
 */
/* TODO: Change most or all pr_info() in this file to pr_debug(), or delete entirely. */
...
/* TODO: Delete this. */
#define I2C_PSEUDO_ENABLE_NATIVE_SMBUS
#undef I2C_PSEUDO_ENABLE_NATIVE_SMBUS
...
#if 0
/* Maximum number of expected controller write commands to allow enqueued at once. */
#define I2C_PSEUDO_CTRLR_CMD_QUEUE_LIMIT (1<<8)
#endif
...
struct i2c_pseudo_cmd {
	 ...
	 *   system after this function returns.
	 *
	 */
...
#ifdef I2C_PSEUDO_ENABLE_NATIVE_SMBUS
struct i2c_pseudo_rsp_smbus_xfer {
	/* TODO: audit these fields when implementing i2c_pseudo_adapter_smbus_xfer() */
	/* These types match those of struct i2c_algorithm.smbus_xfer args. */
	u16 addr;
	...
	/*
	 * Always initialize fields below here to zero.  They are for internal
	 * use by i2c_pseudo_rsp_smbus_xfer_formatter().
	 */
...
static ssize_t i2c_pseudo_rsp_master_xfer_formatter(void *data, char **out)
{
	...
	if (mxfer_rsp->num_msgs_done >= mxfer_rsp->num) {
		pr_info("%s: Returning zero after matching (mxfer_rsp->num <= "
			"0 || mxfer_rsp->num_msgs_done >= mxfer_rsp->num) "
			"condition.  mxfer_rsp->num=%d "
			"mxfer_rsp->num_msgs_done=%d\n", __func__,
			mxfer_rsp->num, mxfer_rsp->num_msgs_done);
		ret = 0;
	...
	if (mxfer_rsp->buf_start_plus_one >= 1 &&
		...
		pr_info("%s: Incrementing mxfer_rsp->num_msgs_done and "
...
#if 0
	if (mxfer_rsp->buf_start_plus_one >= i2c_msg->len + 1) {
		pr_err("%s: mxfer_rsp->buf_start_plus_one >= i2c_msg->len+1 "
			"(%zu >= %u+1).  This is a bug.\n", __func__,
			mxfer_rsp->buf_start_plus_one, i2c_msg->len);
		ret = -ENOTRECOVERABLE;
		goto maybe_free_and_definitely_return;
	}
#endif
	...
	if (mxfer_rsp->buf_start_plus_one <= 0) {
		pr_info("%s: Condition (mxfer_rsp->buf_start_plus_one <= 0) "
			"met, creating read response buffer for I2C message "
			"header.  data=%p mxfer_rsp->num=%d "
			"mxfer_rsp->num_msgs_done=%d "
			"mxfer_rsp->buf_start_plus_one=%zu\n", __func__, data,
			mxfer_rsp->num, mxfer_rsp->num_msgs_done,
			mxfer_rsp->buf_start_plus_one);
		/* TODO: Consider not writing the length, since it is not strictly necessary with the explicit end-of-message marker (i2c_pseudo_ctrlr_end_char). */
		...
		pr_info("%s: anprintf() for I2C message header returned %zd.  "
			"data=%p buf_start=%p\n", __func__, ret, data,
			buf_start);
		if (ret > 0) {
			*out = buf_start;
	...
	pr_info("%s: Attempting to kzalloc() a data bytes read response "
		"buffer.  data=%p mxfer_rsp->num=%d "
		"mxfer_rsp->num_msgs_done=%d mxfer_rsp->buf_start_plus_one=%zu "
		"i2c_msg->addr=0x%04X i2c_msg->flags=0x%04X i2c_msg->len=%u "
		"byte_start=%zu byte_limit=%zu buf_size=%zu\n", __func__, data,
		mxfer_rsp->num, mxfer_rsp->num_msgs_done,
		mxfer_rsp->buf_start_plus_one, i2c_msg->addr, i2c_msg->flags,
		i2c_msg->len, byte_start, byte_limit, buf_size);
	...
	pr_info("%s: Successfully kzalloc()'d a data bytes read response "
		"buffer.  data=%p mxfer_rsp->num=%d "
		"mxfer_rsp->num_msgs_done=%d mxfer_rsp->buf_start_plus_one=%zu "
		"i2c_msg->addr=0x%04X i2c_msg->flags=0x%04X i2c_msg->len=%u "
		"byte_start=%zu byte_limit=%zu buf_size=%zu buf_start=%p\n",
		__func__, data, mxfer_rsp->num, mxfer_rsp->num_msgs_done,
		mxfer_rsp->buf_start_plus_one, i2c_msg->addr, i2c_msg->flags,
		i2c_msg->len, byte_start, byte_limit, buf_size,
		buf_start);
...
#if 0
		pr_info("%s: data bytes read response buffer write loop: buf_start=%p buf_pos=%p i=%zu byte_limit=%zu\n", __func__, buf_start, buf_pos, i, byte_limit);
		pr_info("%s: data bytes read response buffer write loop: buf_start=%p buf_pos=%p i=%zu byte_limit=%zu sep_char=%hhu\n", __func__, buf_start, buf_pos, i, byte_limit, (char)((i || byte_start) ? i2c_pseudo_ctrlr_data_sep_char : i2c_pseudo_ctrlr_header_sep_char));
		pr_info("%s: data bytes read response buffer write loop: buf_start=%p buf_pos=%p i=%zu byte_limit=%zu byte=0x%02X\n", __func__, buf_start, buf_pos, i, byte_limit, i2c_msg->buf[byte_start + i]);
#endif
	...
	pr_info("%s: Successfully wrote data bytes to a read response buffer.  "
		"data=%p mxfer_rsp->num=%d mxfer_rsp->num_msgs_done=%d "
		"mxfer_rsp->buf_start_plus_one=%zu i2c_msg->addr=0x%04X "
		"i2c_msg->flags=0x%04X i2c_msg->len=%u byte_start=%zu "
		"byte_limit=%zu buf_size=%zu buf_start=%p out=%p\n", __func__,
		data, mxfer_rsp->num, mxfer_rsp->num_msgs_done,
		mxfer_rsp->buf_start_plus_one, i2c_msg->addr, i2c_msg->flags,
		i2c_msg->len, byte_start, byte_limit, buf_size, buf_start, out);
	*out = buf_start;
	pr_info("%s: Successfully wrote data byte read response buffer address "
		"to *out.  data=%p mxfer_rsp->num=%d "
		"mxfer_rsp->num_msgs_done=%d mxfer_rsp->buf_start_plus_one=%zu "
		"i2c_msg->addr=0x%04X i2c_msg->flags=0x%04X i2c_msg->len=%u "
		"byte_start=%zu byte_limit=%zu buf_size=%zu buf_start=%p "
		"out=%p\n", __func__, data, mxfer_rsp->num,
		mxfer_rsp->num_msgs_done, mxfer_rsp->buf_start_plus_one,
		i2c_msg->addr, i2c_msg->flags, i2c_msg->len, byte_start,
		byte_limit, buf_size, buf_start, out);
	ret = buf_size;
 ...
 definitely_free_and_definitely_return:
			pr_info("%s: Attempting to kfree(mxfer_rsp->msgs).  "
				"data=%p mxfer_rsp->msgs=%p mxfer_rsp->num=%d "
				"mxfer_rsp->num_msgs_done=%d "
				"mxfer_rsp->buf_start_plus_one=%zu\n", __func__,
				data, mxfer_rsp->msgs, mxfer_rsp->num,
				mxfer_rsp->num_msgs_done,
				mxfer_rsp->buf_start_plus_one);
			kfree(mxfer_rsp->msgs);
			pr_info("%s: Attempting to kfree(mxfer_rsp).  "
				"data=%p\n", __func__, data);
			kfree(mxfer_rsp);
		/*
		 ...
		 */
			...
			pr_info("%s: Attempting to kfree(mxfer_rsp->msgs) and "
				"set it NULL before mxfer_rsp is fully "
				"consumed.  data=%p mxfer_rsp->msgs=%p "
				"mxfer_rsp->num=%d mxfer_rsp->num_msgs_done=%d "
				"mxfer_rsp->buf_start_plus_one=%zu\n", __func__,
				data, mxfer_rsp->msgs, mxfer_rsp->num,
				mxfer_rsp->num_msgs_done,
				mxfer_rsp->buf_start_plus_one);
...
#if 0
	pr_info("%s(%p, %p) returning %zd\n", __func__, data, out, ret);
#endif
	return ret;
}

#ifdef I2C_PSEUDO_ENABLE_NATIVE_SMBUS
static ssize_t i2c_pseudo_rsp_smbus_xfer_formatter(void *data, char **out)
{
	ssize_t ret;
	struct i2c_pseudo_rsp_smbus_xfer *smxfer_rsp;

	smxfer_rsp = data;
	ret = 0;  /* TODO: Actually implement this function. */

 maybe_free_and_definitely_return:
	if (ret <= 0) {
		kfree(smxfer_rsp->data);
		kfree(smxfer_rsp);
	}
	return ret;
}
	...
	size_t in_size, bool non_blocking)
{
	...
	cmd_data = data;
	pr_info("%s called for data=%p in=%p in_size=%zu in=\"%*pE\"\n",
		__func__, data, in, in_size, (int)in_size, in);
	...
	pr_info("%s successfully acquired cmd_data->reply_queue_lock for "
		"cmd_data=%p list_empty(&cmd_data->reply_queue_head)=%d "
		"cmd_data->next_mxfer_id=%u in=%p in_size=%zu in=\"%*pE\"\n",
		__func__, cmd_data, list_empty(&cmd_data->reply_queue_head),
		cmd_data->next_mxfer_id, in, in_size, (int)in_size, in);
	...
	int receive_status, bool non_blocking)
{
	...
	cmd_data = data;
	pr_info("%s called for data=%p receive_status=%d\n", __func__, data,
		receive_status);
	...
	pr_info("%s successfully acquired cmd_data->reply_queue_lock for "
		"cmd_data=%p list_empty(&cmd_data->reply_queue_head)=%d "
		"cmd_data->next_mxfer_id=%u receive_status=%d\n",
		__func__, cmd_data, list_empty(&cmd_data->reply_queue_head),
		cmd_data->next_mxfer_id, receive_status);
	mxfer_reply = list_empty(&cmd_data->reply_queue_head) ? NULL :
		list_first_entry(&cmd_data->reply_queue_head,
			struct i2c_pseudo_cmd_mxfer_reply, reply_queue_item);
	if (mxfer_reply != NULL) {
	...
	i2c_msg = &mxfer_reply->msgs[mxfer_reply->current_msg_idx];
	pr_info("%s: mxfer_reply=%p mxfer_reply->id=%u mxfer_reply->ret=%d "
		"mxfer_reply->num_msgs=%d mxfer_reply->state=%d "
		"mxfer_reply->current_addr=0x%04X "
		"mxfer_reply->current_msg_idx=%d "
		"mxfer_reply->current_buf_idx=%u i2c_msg->addr=0x%04X "
		"i2c_msg->flags=0x%04X i2c_msg->len=%u\n", __func__,
		mxfer_reply, mxfer_reply->id, mxfer_reply->ret,
		mxfer_reply->num_msgs, mxfer_reply->state,
		(unsigned int)mxfer_reply->current_addr,
		mxfer_reply->current_msg_idx,
		(unsigned int)mxfer_reply->current_buf_idx,
		(unsigned int)i2c_msg->addr, (unsigned int)i2c_msg->flags,
		(unsigned int)i2c_msg->len);
		...
		struct i2c_msg *msgs, int num)
{
	...
	struct i2c_pseudo_cmd_mxfer_reply_data *cmd_data;
	struct i2c_pseudo_cmd_mxfer_reply *mxfer_reply;
	pr_info("i2c_pseudo_adapter_master_xfer(adap=%p, msgs=%p, num=%d) "
		"called for I2C adapter %d\n", adap, msgs, num, adap->nr);
...
#if 0
	/* TODO: Consider returning -EBADE when the controller response to an I2C read is invalid.  Or stick with -EIO. */
	/* TODO: Implement support for reads and delete this. */
	for (i = 0; i < num; ++i) {
		if (msgs[i].flags & I2C_M_RD) {
			pr_err("%s: An I2C read using I2C adapter %d was "
				"requested.  The I2C pseudo adapter driver "
				"does not support reads yet.\n", __func__,
				adap->nr);
			ret = -EOPNOTSUPP;
			goto just_return;
		}
	...
	if (pdata->read_rsp_queue_length >= I2C_PSEUDO_CTRLR_RSP_QUEUE_LIMIT) {
		ret = -ENOBUFS;
#if 0
		pr_info("%s: I2C pseudo controller for I2C adapter %d has "
	...
	pr_info("%s: For I2C pseudo adapter %d list_empty("
		"&pdata->read_rsp_queue_head) returns %d after list_add_tail() "
		"and pdata->read_rsp_queue_length now incremented to %u.\n",
		__func__, adap->nr, list_empty(&pdata->read_rsp_queue_head),
	...
	if (wait_ret == -ERESTARTSYS) {
		...
		ret = wait_ret >= INT_MIN ? wait_ret : -ENOTRECOVERABLE;
		...
		if (wait_ret == 0)
			...
			pr_info("%s: I2C adapter %d data_filled completion "
...
#ifdef I2C_PSEUDO_ENABLE_NATIVE_SMBUS
static int i2c_pseudo_adapter_smbus_xfer(struct i2c_adapter *adap, u16 addr,
		unsigned short flags, char read_write, u8 command, int size,
		union i2c_smbus_data *data)
{
	struct i2c_pseudo_controller *pdata;
	pr_info("i2c_pseudo_adapter_smbus_xfer(adap=%p, addr=0x%02X, "
		"flags=%hu, read_write=%hhu, command=%hu, size=%d, data=%p) "
		"called for I2C adapter %d\n", adap, (unsigned int)addr, flags,
		(unsigned char)read_write, (unsigned short)command, size, data,
		adap->nr);
	pdata = adap->algo_data;
	/* TODO: thread-safety */
...
#ifdef I2C_PSEUDO_ENABLE_NATIVE_SMBUS
	.smbus_xfer = i2c_pseudo_adapter_smbus_xfer,
#endif
...
static int i2c_pseudo_cdev_open(struct inode *inodep, struct file *filep)
{
	...
	struct i2c_pseudo_controller *pdata;
	pr_info("i2c_pseudo_cdev_open(inodep=%p, filep=%p) called\n", inodep,
		filep);
 ...
 unlock_counters:
	...
	pr_info("%s: i2c_add_adapter() for inodep=%p filep=%p successfully "
		"added I2C adapter %d with name \"%*pE\"\n", __func__, inodep,
		filep, pdata->i2c_adapter.nr, (int)strnlen(
			pdata->i2c_adapter.name,
...
static int i2c_pseudo_cdev_release(struct inode *inodep, struct file *filep)
{
	...
	pr_info("i2c_pseudo_cdev_release(inodep=%p, filep=%p) called for I2C "
		"adapter %d\n", inodep, filep, pdata->i2c_adapter.nr);
		...
		size_t count, loff_t *f_ps)
{
	...
	pdata = filep->private_data;
	pr_info("i2c_pseudo_cdev_read(filep=%p, buf=%p, count=%zu, f_ps=%p) "
		"with non_blocking=%u called for I2C adapter %d\n", filep, buf,
		count, f_ps, (unsigned int)non_blocking, pdata->i2c_adapter.nr);
	...
	if (non_blocking) {
		if (!mutex_trylock(&pdata->rsp_lock)) {
			BUILD_BUG_ON(ret != 0);
			ret = -EAGAIN;
			pr_info("%s: Could not immediately acquire I2C adapter "
	...
	if (pdata->rsp_buf_remaining < 0) {
		BUILD_BUG_ON(ret != 0);
#if 0
		pr_info("%s: Error code %zd left in "
			"pdata->rsp_buf_remaining for I2C pseudo adapter %d, "
			"will return that error now.\n", __func__,
			pdata->rsp_buf_remaining, pdata->i2c_adapter.nr);
#endif
	...
	while (count > 0) {
		...
		while (pdata->rsp_buf_remaining == 0) {
#if 0
			pr_info("%s: Looking for current read response "
				"formatter for I2C pseudo controller of I2C "
				"adapter %d with "
				"pdata->rsp_invalidated=%u.\n", __func__,
				pdata->i2c_adapter.nr,
				(unsigned int)pdata->rsp_invalidated);
#endif
			/*
			...
			if (!list_empty(&pdata->read_rsp_queue_head))
				...
				pr_info("%s: Encountered empty read response "
					"queue for I2C pseudo controller of "
					"I2C adapter %d.  "
					"pdata->read_rsp_queue_length=%u\n",
			...
			if (rsp_wrapper == NULL) {
				...
				pr_err("%s: I2C adapter %d "
					"pdata->read_rsp_queue_head was empty "
					"after achieving pdata->read_rsp_queued"
					" completion.  This is a bug.  "
					"Returning %zd now.\n", __func__,
					pdata->i2c_adapter.nr, ret);
				goto unlock_and_return;
			}
...
#if 0
			pr_info("%s: rsp_wrapper->formatter() returned %zd and "
				"set pdata->rsp_buf_start to %p\n", __func__,
				pdata->rsp_buf_remaining, pdata->rsp_buf_start);
#endif
	...
	bool non_blocking)
{
	...
	pr_info("%s(%p, %p, %zu) called for buffer \"%*pE\" with existing "
		"pdata->cmd_size=%zu and pdata->cmd_buf=\"%*pE\"\n", __func__,
		pdata, start, remaining, (int)remaining, start, pdata->cmd_size,
		(int)pdata->cmd_size, pdata->cmd_buf);
	...
	pr_info("%s: Will copy %zu header bytes from the %zu remaining "
		"write bytes in this loop iteration, taking presence "
		"or absence of any '%1pE' command end delimiter or "
		"'%1pE' header separator character into account.\n",
	...
	if (cmd_idx < 0) {
		...
		if (i >= ARRAYLEN(i2c_pseudo_cmds)) {
			ret = -EINVAL;
			pr_info("%s: unrecognized command \"%*pE\"\n", __func__,
				(int)pdata->cmd_size, pdata->cmd_buf);
 ...
 just_return_ret:
	if (ret < 0 && !pdata->cmd_receive_status)
	...
	bool non_blocking)
{
	...
	pr_info("%s: Will copy %zu data bytes from the %zu remaining "
		"write bytes in this loop iteration, unless '%1pE' "
		"command end delimiter is found first.\n", __func__,
		size_holder, remaining, &i2c_pseudo_ctrlr_end_char);
	...
	pr_info("%s: Will copy %zu data bytes from the %zu remaining "
		"write bytes in this loop iteration, taking presence "
		"or absence of any '%1pE' command end delimiter into "
		...
		size_t count, loff_t *f_ps)
{
	 ...
	 * expected, it arguably makes more sense to indicate an error than to
	 * block until possibly receiving a master_xfer request for that I2C
	 * message.
	 */
	non_blocking = filep->f_flags & O_NONBLOCK;
	pdata = filep->private_data;
	pr_info("%s(filep=%p, buf=%p, count=%zu, f_ps=%p) with non_blocking=%u "
		"called for I2C adapter %d\n", __func__, filep, buf, count,
		f_ps, (unsigned int)non_blocking, pdata->i2c_adapter.nr);
	...
	pr_info("%s(filep=%p, buf=%p, count=%zu, f_ps=%p) called for buffer "
		"\"%*pE\"\n", __func__, filep, buf, count, f_ps, (int)count,
		kbuf);
	...
	if (mutex_lock_killable(&pdata->cmd_lock)) {
		ret = -EINTR;
		pr_warn("%s: I2C adapter %d cmd_lock acquisition interrupted "
			"by signal, returning %zd.\n", __func__,
			pdata->i2c_adapter.nr, ret);
		goto free_and_return;
	}
...
static __poll_t i2c_pseudo_cdev_poll(struct file *filep, struct poll_table_struct *pt)
{
	struct i2c_pseudo_controller *pdata;

	pdata = filep->private_data;

	/* TODO: finish implementing this function(ality) */
}
...
static int __init i2c_pseudo_init(void)
{
	...
	pr_info("%s: alloc_chrdev_region(&dev, " STR(I2C_PSEUDO_CDEV_BASEMINOR)
		", " STR(I2C_PSEUDO_CDEV_COUNT) ", " STR(I2C_PSEUDO_CHRDEV_NAME)
		") returned %d and set MAJOR(dev)==%u MINOR(dev)=%u\n",
		__func__, ret, MAJOR(i2c_pseudo_dev_num),
		MINOR(i2c_pseudo_dev_num));
...
inline static void i2c_pseudo_build_assertions(void)
{
	...
	BUILD_BUG_ON(ARRAYLEN(i2c_pseudo_cmds) != I2C_PSEUDO_NUM_REPLY_CMDS);
	/*
