...
static void nivviks_subboard_init(void)
{
	...
	if (sb != NISSA_SB_C_A && sb != NISSA_SB_HDMI_A) {
		...
		gpio_pin_configure_dt(GPIO_DT_LABEL(gpio_sub_usb_a1_ilimit_sdp),
				      GPIO_DISCONNECTED);
		gpio_pin_configure_dt(GPIO_DT_LABEL(gpio_en_sub_usb_a1_vbus),
				      GPIO_DISCONNECTED);
	}
	if (sb == NISSA_SB_C_A || sb == NISSA_SB_C_LTE) {
		/* Enable type-C port 1 */
		gpio_pin_configure_dt(GPIO_DT_LABEL(gpio_usb_c1_int_odl),
				      GPIO_INPUT);
	} else {
	...
	if (sb == NISSA_SB_HDMI_A) {
		...
		/* Enable HDMI GPIOs */
		gpio_pin_configure_dt(GPIO_DT_LABEL(gpio_en_sub_rails_odl),
				      GPIO_OUTPUT_HIGH);
		gpio_pin_configure_dt(GPIO_DT_LABEL(gpio_hdmi_en_sub_odl),
				      GPIO_OUTPUT_HIGH);
		/* Configure the interrupt separately */
		gpio_pin_configure_dt(GPIO_DT_LABEL(gpio_hpd_sub_odl),
				      GPIO_INPUT);
	}
}
