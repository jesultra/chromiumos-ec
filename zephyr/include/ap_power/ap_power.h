...
/**
 * @brief AP events for callback notification.
 */
enum ap_events {
	AP_EV_PRE_INIT = BIT(0),
	AP_EV_STARTUP = BIT(1),
	AP_EV_RESUME = BIT(2),
	AP_EV_SUSPEND = BIT(3),
	AP_EV_RESUME_INIT = BIT(4),
	AP_EV_SUSPEND_COMPLETE = BIT(5),
	AP_EV_SHUTDOWN = BIT(6),
	AP_EV_SHUTDOWN_COMPLETE = BIT(7),
	AP_EV_HARD_OFF = BIT(8),
	AP_EV_RESET = BIT(9),
};
 ...
 */
struct ap_ev_data {
	enum ap_events event;
	/* May need more data here */
};

struct ap_ev_callback;

/**
 * @brief Callback handler definition
 */
typedef void (*ap_ev_callback_handler_t)(struct ap_ev_callback *cb,
					 struct ap_ev_data data);

/**
 * @cond INTERNAL_HIDDEN
 *
 * Register a callback for the AP events requested.
 * As many callbacks as needed can be added as long as each of them
 * are unique pointers of struct ap_ev_callback.
 * The storage must be static.
 *
 * ap_ev_init_callback can be used to initialise this structure.
 */
struct ap_ev_callback {
	sys_snode_t node;	/* Only usable by AP power code */
	ap_ev_callback_handler_t handler;
	enum ap_events events;	/* Events to listen for */
};
/** @endcond */

/**
 * @brief Initialise a struct ap_ev_callback properly.
 *
 ...
 */
static inline void ap_ev_init_callback(struct ap_ev_callback *cb,
				       ap_ev_callback_handler_t handler,
				       enum ap_events events)
{
 ...
 */
void ap_ev_add_events(struct ap_ev_callback *cb, enum ap_events events);
 ...
 */
static inline void ap_ev_remove_events(struct ap_ev_callback *cb,
				       enum ap_events events)
{
	__ASSERT(cb, "Callback pointer should not be NULL");
