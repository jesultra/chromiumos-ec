...
#define TEMP_AMBIENT TEMP_SENSOR_ID(DT_NODELABEL(temp_ambient))
	...
	if (temp[TEMP_MEMORY] < prev_tmp[TEMP_MEMORY] ||
		...
		for (i = current_level; i < ARRAY_SIZE(fan_step_table); i++) {
			if (temp[TEMP_MEMORY] >
			    ...
			    (temp[TEMP_AMBIENT] >
				     fan_step_table[i].on[TEMP_AMBIENT] &&
			     temp[TEMP_CHARGER] >
