# Cr50 And Chrome OS Verified Boot Problems
...
In Chromeboxes H1 is rebooted every time the system is powered up.

One of the important functions of H1 in the system is TPM (Trusted Platform
Module). The TPM stores verified boot information, this is why any **problems
communicating with the TPM during the boot up process** result in the Chrome
OS device falling into **recovery mode**.
...
The H1 never enters the *deep sleep* state during the Chrome OS boot up
process, but could enter the *sleep* state if the Chrome OS device boot
process is delayed for whatever reason, and **only when CCD is not active**.
This could be one of the reasons that there are boot failures when CCD is not
connected, but the failures go away if CCD is on (the debug cable is plugged
in).
...
Both I2C and SPI interfaces do not fully comply with the standards: the I2C
controller does not support clock stretching, and the SPI controller can not
be clocked faster than 2 MHz.
...
SOCs have a bidirectional reset signal called `SYS\_RST\_L`. They (or, rather,
...
SOC asynchronosly, this is what the Cr50 does to reset ARM SOCs.

The X86 SOCs have two separate signals, one output `PLT\_RST\_L` which is held
low, while the AP is in reset or in low power mode, and one input,
`SYS\_RST\_L` which Cr50 toggles to reset the SOC.

In case of X86, when `PLT\_RST\_L` is held low longer than a certain time (a
second or so), Cr50 considers this an indication of the AP going into a low
power mode (S5 or lower), which means that the AP will start from the reset
vector when it wakes up, so Cr50 can take H1 into *deep sleep* mode as well.

On top of that ARM based Chrome OS devices have some additional logic which
forces the `SYS\_RST\_L` behave similar to `PLT\_RST\_L` - it stays low when
...
In the `brdp` command output bit D5 when set signifies `SYS\_RST\_L `('regular'
ARM devices) and bit D6 - `PLT\_RST\_L` (X86 and modified ARM) type of reset.

Boot problems can arise when the AP reboots, but the Cr50 misses the fact that
the AP was in reset: in this case the very first TPM_Startup command sent by
coreboot returns an error, and the Chrome OS device falls into recovery mode.
...
to the AP that the H1 is ready for the next I2C or SPI transaction. This
signal is a low 8 us long pulse. Some X86 platforms require a longer pulse,
pulse extension can be achieved by writing into a Cr50 register.
...
ready. This is why in case the previousSPI transaction was a second or more
...
control the fast master controller.
...
Specification](https://drive.google.com/file/d/16r1vDhf1fnggI4BkOBuTXPqOQt4LaFvk/view?usp=sharing)
