...
hc_regulator_get_info(struct host_cmd_handler_args *args)
{
	...
	CPRINTS("GET INFO %u", p->index);

	/*
	 * TODO(pihsun): How to ensure that the name / voltage_uV array are not
	 * written out of bound?
	 */
...
hc_regulator_get_voltage(struct host_cmd_handler_args *args)
{
	...
	rv = board_regulator_get_voltage(p->index, &r->voltage_uV);

	if (rv)
		return EC_RES_ERROR;

	CPRINTS("RETURNS %u", r->voltage_uV);
	args->response_size = sizeof(*r);
	return EC_RES_SUCCESS;
}
...
hc_regulator_set_voltage(struct host_cmd_handler_args *args)
{
	...
	CPRINTS("SET REGULATOR %u (%u - %u)", p->index, p->min_uV, p->max_uV);

	rv = board_regulator_set_voltage(p->index, p->min_uV, p->max_uV);
