		       ...
		       uint16_t header, const uint32_t *data)
{
	...
	if ((pd[port].rev == PD_REV30) &&
		...
		if (pd[port].power_role == PD_ROLE_SOURCE) {
			...
			/*
			 * Allow the Sink to complete its transmission.
			 * Wait until PD_EVENT_RX is received or
			 * PD_T_SINK_TX timeout.
			 */
			evt = task_wait_event_mask(PD_EVENT_RX, PD_T_SINK_TX);

			/*
			 * If set, reset the PD_EVENT_RX event so it can be
			 * handled elsewhere.
			 */
			if (evt & PD_EVENT_RX)
			...
			/*
			 * A Sink can still send Hard Reset signaling at any
			 * time.
			 */
...
static int send_battery_cap(int port, uint32_t *payload)
{
	...
	if (battery_is_present() && (payload != NULL)) {
		/* Get Battery Cap Ref */
		bat_cap_ref = *((uint8_t *)payload + 2);

		/* We only have one fixed battery, so this should be 0 */
		if (bat_cap_ref != 0) {
			/* Invalid battery reference */
			msg[5] = 1;
		} else {
			/* Get battery voltage (mV) */
			ret_v = battery_design_voltage(&v);
			/* mV -> V */
			v = DIV_ROUND_NEAREST(v, 1000);
			 ...
			 */
			/* Get battery design capacity (mAh) */
			ret_c = battery_design_capacity(&c);
			/* mAh -> Ah */
			c = DIV_ROUND_NEAREST(c, 1000);
			 ...
			 */
			/* Get battery full capacity (mAh) */
			ret_f = battery_full_charge_capacity(&f);
			/* mAh -> Ah */
			f = DIV_ROUND_NEAREST(f, 1000);

			/* Set Battery Design Capacity 1/10 WH */
			if (ret_c < 0 || ret_v < 0)
				msg[3] = 0xffff;
			else
				/* Wh -> 10th of a Wh */
				msg[3] = DIV_ROUND_NEAREST((c * v), 10);

			/* Set Battery Full Capacity 1/10 WH */
			if (ret_f < 0 || ret_v < 0)
				msg[4] = 0xffff;
			else
				/* Wh -> 10th of a Wh */
				msg[4] = DIV_ROUND_NEAREST((f * v), 10);
		}
	}
...
static int send_battery_status(int port,  uint32_t *payload)
{
	...
	uint16_t header = PD_HEADER(PD_DATA_BATTERY_STATUS,
				    ...
				    pd[port].rev,
				    0 /* This is NOT an exteded message */
				  );
	...
	if (battery_is_present() && (payload != NULL)) {
		...
		if (bat_cap_ref != 0) {
			...
			ret_c = battery_remaining_capacity(&c);
			/* mAh -> Ah */
			c = DIV_ROUND_NEAREST(c, 1000);

			/* set battery capacity 1/10 WH */
			if (ret_v < 0 || ret_c < 0)
				msg |= BSDO_CAP(BSDO_CAP_UNKNOWN);
			else
				msg |= BSDO_CAP(DIV_ROUND_NEAREST((c * v), 10));

			/* Battery is present */
			msg |= BSDO_PRESENT;

			if (battery_status(&s) < 0) {
		...
		/* Battery state of charge unknown */
		msg = BSDO_CAP(BSDO_CAP_UNKNOWN);
	}
...
#ifdef CONFIG_USB_PD_REV30
	int snk_rev = PD_HEADER_REV(head);
#endif
...
#ifdef CONFIG_USB_PD_REV30
	/* Check if this is an exteded chunked data message. */
