...
static void ccd_measure_sbu(void)
{
	...
	/*
	 * If SuzyQ is enabled, we'll poll for a persistant no-signal for
...
static void board_init(void)
{
	...
	/*
	 * Disable SBU mux. The polarity is set each time a new PD attach event
	 * occurs. But, the SBU mux is not disabled on detach so that the H1 USB
	 * console will survie a DUT EC reset. Explain more.
	 */
	gpio_set_level(GPIO_SBU_MUX_EN, 0);
