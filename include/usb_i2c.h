 ...
 *     common USB packets (64 bytes), remaining payload should send without
 *     header.
 *
 *   - Write at most 60 bytes and read no more than 60 bytes.
 *     For usage beyond those, please redefine CONFIG_USB_I2C_MAX_WRITE_COUNT /
 *     CONFIG_USB_I2C_MAX_READ_COUNT and use header extended protocol in next.
 *   +------+------+----+----+---------------+
 *   | port | addr | wc | rc | write payload |
 *   +------+------+----+----+---------------+
 *   |  1B  |  1B  | 1B | 1B |  <= 60 bytes  |
 *   +------+------+----+----+---------------+
 *
 *   - Write at most 256 bytes and read no more than 32767 (0x7FFF) bytes.
 *   +------+------+----+----+-----+----------+---------------+
 ...
 *     that we want to read back more than 60 bytes, so the first byte of
 ...
 *         than 60 bytes please see the special notes above.
 *
 ...
 *          count (rc >= 0x80), rc1 will concatenate wich rc togeter. See the
