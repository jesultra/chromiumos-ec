...
{
	if (argc < 3) {
		fprintf(stderr, "Usage: %s rwsig action [abort | continue]\n",
			argv[0]);
		return -1;
	}

	return rwsig_action(argv[2]);
}
...
static int cmd_rwsig_info(int argc, char *argv[])
{
	...
	if (strcmp(argv[1], "dump") == 0) {
		...
		for (i = 0; i < ARRAY_SIZE(cmd_map); i++)
			if (strcmp(argv[2], cmd_map[i].cmd) == 0)
...
int cmd_rwsig(int argc, char **argv)
{
	...
	struct rwsig_subcommand rwsig_subcommands[] = {
		{ "info", cmd_rwsig_info },
		{ "dump", cmd_rwsig_info },
	...
	for (i = 0; i < ARRAY_SIZE(rwsig_subcommands); i++)
		if (strcmp(argv[1], rwsig_subcommands[i].subcommand) == 0)
			return rwsig_subcommands[i].handler(argc, argv);
