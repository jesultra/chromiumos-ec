...
""")
  ...
  def setup_srctree(self, srctree):
  ...
  def setup_allowed_and_configs(self, allowed_fname, configs_fname):
    ...
    with tempfile.TemporaryDirectory() as srctree:
      ...
      with tempfile.NamedTemporaryFile() as allowed:
        with tempfile.NamedTemporaryFile() as configs:
          ...
          result = checker.find_new_adhoc_configs(configs.name, srctree,
                                                  allowed.name, PREFIX)
    ...
    with capture_sys_output() as (stdout, stderr):
      with tempfile.TemporaryDirectory() as srctree:
        ...
        with tempfile.NamedTemporaryFile() as allowed:
          with tempfile.NamedTemporaryFile() as configs:
            self.setup_allowed_and_configs(allowed.name, configs.name)
            ret_code = kconfig_check.main(['-c', configs.name, '-s', srctree,
                                           '-a', allowed.name, '-p', PREFIX,
                                            'check'])
