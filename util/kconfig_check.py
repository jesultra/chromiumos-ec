...
a corresponding Kconfig option for Zephyr"""
  ...
  parser.add_argument('-s', '--srctree', type=str,
...
class KconfigCheck:
  ...
  def find_new_adhoc(self, configs, kconfigs, allowed):
    """Get a list of new ad-hoc CONFIG options

    Arguments and return value should omit the 'CONFIG_' prefix, so CONFIG_LTO
    should be provided as 'LTO'.
  ...
  def read_configs(self, configs_file):
    ...
    Returns:
      List of CONFIG_xxx options found in the file, with the 'CONFIG_' prefix
        removed
    """
  ...
  def find_kconfigs(self, srcdir):
    """Find all the Kconfig files in a source directory, recursively

    Any subdirectory called 'Kconfig' is ignored, since Zephyr generates this in
    its build directory.
  ...
  def scan_kconfigs(self, srcdir, prefix=''):
    ...
    Args:
      srcdir: Directory to scan
      prefix: Prefix to string from the name (e.g. 'PLATFORM_EC_')
    ...
    for fname in self.find_kconfigs(srcdir):
      with open(fname) as inf:
        found = re.findall(
          '^(config|menuconfig) %s([A-Za-z0-9_]*)' % prefix,
          inf.read())
        kconfigs += [name for kctype, name in found]
    return kconfigs
  ...
  def do_check(self, configs_file, srcdir, allowed_file, prefix):
    ...
    """
    new_adhoc = self.find_new_adhoc_configs(configs_file, srcdir, allowed_file,
                                            prefix)
    if new_adhoc:
