...
static timestamp_t ts[PD_PORT_COUNT][TRANSITION_COUNT];
static int ts_idx[PD_PORT_COUNT];
uint64_t get_time_since_last_edge(int port)
{
	int prev_idx = (ts_idx[port] == 0) ? TRANSITION_COUNT-1 :
					     ts_idx[port]-1;
	return get_time().val - ts[port][prev_idx].val;
}
...
void pd_rx_handler(void)
{
	...
	for (i = 0; i < PD_PORT_COUNT; i++) {
		if (pending & EXTI_COMP_MASK(i)) {
			ts[i][ts_idx[i]].val = get_time().val;
			next_idx = (ts_idx[i] == TRANSITION_COUNT - 1) ?
					0 : ts_idx[i] + 1;

#ifndef CONFIG_COMMON_RUNTIME
			/* Do not deep sleep while waiting for more edges */
			 ...
			 */
			if ((ts[i][ts_idx[i]].val - ts[i][next_idx].val)
			    < TRANSITION_WINDOW) {
				/* start sampling */
				pd_rx_start(i);
				/*
