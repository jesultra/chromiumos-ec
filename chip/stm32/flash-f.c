...
static int wait_busy(void)
{
	...
	while ((STM32_FLASH_SR & STM32_FLASH_SR_BSY) && timeout-- > 0)
...
static int unlock(int locks)
{
	...
	if (STM32_FLASH_CR & STM32_FLASH_CR_LOCK) {
		STM32_FLASH_KEYR = STM32_FLASH_KEY1;
		STM32_FLASH_KEYR = STM32_FLASH_KEY2;
	}
	/* unlock option memory if required */
	if ((locks & OPT_LOCK) && STM32_FLASH_OPT_LOCKED) {
		STM32_FLASH_OPTKEYR = STM32_FLASH_OPTKEY1;
		STM32_FLASH_OPTKEYR = STM32_FLASH_OPTKEY2;
	}
...
static int write_optb(uint32_t mask, uint32_t value)
{
	...
	STM32_FLASH_OPTCR |= STM32_FLASH_OPTSTRT;
...
static int erase_optb(void)
{
	...
	STM32_FLASH_CR |= STM32_FLASH_CR_OPTER;
...
static int write_optb(int byte, uint8_t value)
{
	...
	/* set OPTPG bit */
	STM32_FLASH_CR |= STM32_FLASH_CR_OPTPG;
...
int flash_physical_write(int offset, int size, const char *data)
{
	...
	uint8_t *address = (uint8_t *)(CONFIG_FLASH_BASE + offset);
	uint8_t quantum = 0;
...
#error "CONFIG_FLASH_WRITE_SIZE not supported."
	...
	STM32_FLASH_SR = STM32_FLASH_SR_ALL_ERR | STM32_FLASH_SR_EOP;
	...
	for (; size > 0; size -= CONFIG_FLASH_WRITE_SIZE) {
		...
		/* wait to be ready  */
		for (i = 0;
		     (STM32_FLASH_SR & STM32_FLASH_SR_BSY) &&
		...
		*address++ = quantum;
		...
		if (STM32_FLASH_SR & STM32_FLASH_SR_BSY) {
		 ...
		 * protection error */
		if (STM32_FLASH_SR & STM32_FLASH_SR_ALL_ERR) {
			res = EC_ERROR_UNKNOWN;
			goto exit_wr;
		}
	}
...
int flash_physical_erase(int offset, int size)
{
	...
	int timeout_us;
#ifdef CHIP_FAMILY_STM32F4
	int sector = flash_bank_offset(offset);
	/* we take advantage of sector_size == erase_size */
	if ((sector < 0) || (flash_bank_offset(offset + size) < 0))
	...
	while (size > 0) {
		...
		watchdog_reload();
		while ((STM32_FLASH_SR & STM32_FLASH_SR_BSY) &&
...
int flash_physical_protect_at_boot(enum flash_wp_range range)
{
	 ...
	 */
	STM32_OPTB_RDP_USER = STM32_OPTB_RDP_USER &
		~(0xFF << (STM32_OPTB_RDP_OFF*8) | 0x1100;
#endif
...
static int registers_need_reset(void)
{
	...
	int ro_wp_region_end = RO_BANK_OFFSET + RO_BANK_COUNT;
